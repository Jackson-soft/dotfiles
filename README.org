...
*** ~delta~
[[https://github.com/dandavison/delta][delta]] 是文件对比工具，主要是用来替换 ~git diff~ 用的。实际配置见 [[file:gitconfig.inc][gitconfig.inc]]：
#+begin_src conf
[core]
    pager = delta
...
- ~syntax-theme~ 跟 ~fzf~/~bat~ 统一用 gruvbox，视觉上保持一致；
- ~true-color~/~hyperlinks~ 保证颜色和可点击文件链接在 ~wezterm~ 下正常显示；
- ~relative-paths~ 在子目录下运行 git 时显示相对路径，更好读；
- ~conflictstyle = zdiff3~ 比默认的 ~diff3~ 多显示共同祖先上下文，冲突更好排查（需要 git ≥ 2.35）。

如果是使用 ~git-fuzzy~ 的话，是不用配置这些的。

*** ~ripgrep~
[[https://github.com/BurntSushi/ripgrep][ripgrep]] 是一个文本搜索工具，配置里用 ~alias grep='rg'~ 替换了默认的 ~grep~ 。

*** ~ugrep~
[[https://github.com/Genivia/ugrep][ugrep]] 同样是一个搜索工具。可以是 ~grep~ 的平替，且带有 ~TUI~ 。

*** =procs=
[[https://github.com/dalance/procs][procs]] 是 ~ps~ 命令的替代品。没有别名替换 ~ps~ ：~ps~ 的经典 flag 语法（~-u~/~-o~/~-p~ 等）被 [[file:zsh/conf.zsh][conf.zsh]] 里 kill 补全和 =fkill= 依赖，跟 ~procs~ 不兼容，直接用 ~procs~ 这个命令名即可。

*** ~bottom~
[[https://github.com/ClementTsang/bottom][bottom]] 是 ~top~ 命令的替代品，通过 ~zinit~ 从 ~gh-r~ 下载二进制安装，并 ~alias top=btm~ 。
...
*** ~zoxide~
[[https://github.com/ajeetdsouza/zoxide][zoxide]] 是一个快速目录跳转工具，支持模糊匹配和历史纪录：
#+begin_src shell
# --cmd j: 默认的 `zi` 会跟 zinit 自己的 `zi` 别名冲突（zinit 加载时会 alias zi=zinit）
(( $+commands[zoxide] )) && eval "$(zoxide init zsh --cmd j)"
#+end_src

它延伸出两个命令（默认前缀是 ~z~ ，这里改成了 ~j~ 避免跟 zinit 冲突）：
- ~j~ :: 这个应该叫 ~cd~ 命令的智能版；
- ~ji~ :: 这个是配合搜索工具（ ~fzf~ ）的 ~cd~ 命令。
